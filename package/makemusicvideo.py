...
def make_music_video(song_file, clips_folder, output_folder):
    ...
    # Get the total duration of the clips
    total_duration_clips = sum([clip.duration for clip in clips])

    # Repeat clips until the total duration matches the song duration
    while total_duration_clips < audio_clip.duration:
        random.shuffle(clip_files)
        clips += [mp.VideoFileClip(clip_file) for clip_file in clip_files]
        total_duration_clips = sum([clip.duration for clip in clips])
    ...
    # Set the duration of the final video to match the song duration
